...
import { userLoginInput, userSignupInput, UserSignupInput } from './auth.schema.ts';
...
export const loginUser = asyncHandler(async (req: Request, res: Response) => {
	const { username, password } = await userLoginInput.parseAsync(req.body);
	...
	res.status(200).appendHeader('Set-Cookie', sessionCookie).appendHeader('Location', '/').json({
		message: 'Login successfully',
	});
});

export const signUpUser = asyncHandler(async (req: Request<never, never, UserSignupInput, never>, res: Response) => {
	const { username, email, password } = await userSignupInput.parseAsync(req.body);
